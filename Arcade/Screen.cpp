...
void Screen::FillPoly(const vector<Vec2D>& points, const Color color)
{
	if (points.size() > 0)
	{
		...
		for (int pixelY = top; pixelY < bottom; pixelY++)
		{
			...
			sort(nodeXVec.begin(), nodeXVec.end(), less<>());
			...
			for (size_t k = 0; k < nodeXVec.size(); k += 2)
			{
				...
				if (nodeXVec[k + 1] > left)
				{
					...
					for (int pixelX = nodeXVec[k]; pixelX < nodeXVec[k + 1]; ++pixelX) Draw(pixelX, pixelY, color);
				}
			}
		}
	}
...
void Screen::Draw(const AARectangle& rectangle, const Color& color, bool fill, const Color& fillColor)
{
	...
	vector<Vec2D> points = rectangle.GetPoints();
