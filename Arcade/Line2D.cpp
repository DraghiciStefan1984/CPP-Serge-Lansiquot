...
bool Line2D::operator==(const Line2D& line) const { return line.GetP0()==this->GetP0() && line.GetP1() == this->GetP1(); }

float Line2D::MinDistanceFromPoint(const Vec2D& p, bool limitToSegment) const { return p.Distance(ClosestPoint(p, limitToSegment)); }

Vec2D Line2D::ClosestPoint(const Vec2D& p, bool limitToSegment) const
{
	Vec2D p0ToP = p - mP0;
	Vec2D p0ToP1 = mP1 - mP0;
	float magnitude = p0ToP1.Mag2();
	float dotProduct = p0ToP.Dot(p0ToP1);
	float t = dotProduct / magnitude;
	if (limitToSegment) t = fmax(0, fmin(1.0f, t));
	return mP0 + p0ToP1 * t;
}

Vec2D Line2D::MidPoint() const { return Vec2D((mP0.GetX() + mP1.GetX()) / 2.0f, (mP0.GetY() + mP1.GetY()) / 2.0f); }

float Line2D::Slope() const
{
	float dx = mP1.GetX() - mP0.GetX();
	if (fabs(dx) < EPSILON) return 0.0;
