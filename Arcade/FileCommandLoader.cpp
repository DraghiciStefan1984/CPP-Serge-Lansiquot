...
void FileCommandLoader::AddCommand(const Command& command) { mCommands.push_back(command); }
...
bool FileCommandLoader::LoadFile(const std::string& filePath)
{
	...
	while (!inFile.eof())
	{
		...
		if ((commandPos = line.find(":")) != std::string::npos)
		{
			...
			if (dilimitPos == std::string::npos) dilimitPos = line.length();
			else dilimitPos -= 1;
...
std::string FileCommandLoader::ReadString(const ParseFuncParams& params) { return params.line.substr(params.dilimitPos + 1); }

char FileCommandLoader::ReadChar(const ParseFuncParams& params) { return params.line.substr(params.dilimitPos + 1)[0]; }